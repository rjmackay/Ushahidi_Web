			...
			<div class="reports-box">
				...
				if (count($incidents) > 0) { 
					...
					<?php }
						...
						<p>There are no reports <?php echo ($category_title) ? " in $category_title" : ""?>.</p>
						<p>Try browsing other categories:</p>
						<!-- Category Selector -->
							<ul class="report-category-index">
							<?php
							foreach ($categories AS $category)
							{
								if ($category->category_image_thumb)
								{
									?>
									<li>
										<a class="" href="<?php echo url::site(); ?>reports/?c=<?php echo $category->id; ?>">
											<span class="">
												<img src="<?php echo url::base().Kohana::config('upload.relative_directory')."/".$category->category_image_thumb; ?>" height="16" width="16" />
											</span>
											<span class="category-title"><?php echo $localized_categories[(string)$category->category_title];?></span>
										</a>
									</li>
									<?php
								}
								...
								{
									?>
									<li><a class="" href="<?php echo url::site(); ?>reports/?c=<?php echo $category->id; ?>">
								...
								if ($category->children) { ?>
									...
									foreach ($category->children as $child) {
										if ($child->category_image_thumb)
										{
											?>
											<li style="padding-left: 20px;">
												<a class="" href="<?php echo url::site(); ?>reports/?c=<?php echo $child->id; ?>">
													<span class="">
														<img src="<?php echo url::base().Kohana::config('upload.relative_directory')."/".$child->category_image_thumb; ?>" height="16" width="16" />
													</span>
													<span class="category-title"><?php echo $localized_categories[(string)$child->category_title];?></span>
												</a>
											</li>
											<?php
										}
										...
										{
											?>
											<li><a href="<?php echo url::site(); ?>reports/?c=<?php echo $child->id; ?>">
												<span style="background-color:#<?php echo $child->category_color;?>;"></span>
												<span class="category-title"><?php echo $localized_categories[(string)$child->category_title]; ?></span>
											</a></li>
											<?php
										}
